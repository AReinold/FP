...
von $\text{\hbar \omega}$ gequantelt ist. Die mittlere Energie pro Oszillator lässt sich über die Boltzmann-Verteilung:
\begin{equation}
   W(n) = \exp\left( -\frac{n\hbar\omega}{k_\text{B}T} \right)
\end{equation}
bestimmen. Mit dieser lässt sich die Wahrscheinlichkeit, dass ein Oszillator bei Temperatur $T$, welchher im Glechgewicht mit seiner Umgebung ist, eine Energie $n \text{\hbar \omega}$ besitzt beschreiben.
...
\begin{equation}
  C_\text{V, Einstein} = 3R\frac{\hbar^2\omega^2}{k_\text{B}^2T^2}
                          \frac{\exp(\hbar\omega/k_\text{B}T)}{\left[\exp(\hbar\omega/k_\text{B}T)\right-1]^2} \,.
\end{equation}
